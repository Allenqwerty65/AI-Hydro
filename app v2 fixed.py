...
def load_data():
    # Try local CSV first if available (much faster!)
...
def train_models(n_rows):
    ...
    for target, col in [("length", "Growth_Length"), ("rate", "Growth_Rate")]:
        ...
        out[target] = {"model": m, "mae": mae, "r2": r2, "acc": acc, "X_test": Xte, "y_test": yte, "y_pred": yp}
    return out

# ============================================================================
# PAGE SETUP
# ============================================================================
st.set_page_config(page_title="🥬 Lettuce Predictor v2", layout="wide")
...
c1.metric("📏 Length Model Acc.", f"{acc_len:.1f}%")
c2.metric("📏 Length MAE",        f"±{mae_len:.2f} cm")
c3.metric("⚡ Rate Model Acc.",   f"{acc_rate:.1f}%")
c4.metric("🗓️ Full Cycle",        f"{MAX_DAYS} days")
st.divider()
...
with tab1:
    ...
    with col_r:
        ...
        if go:
            ...
            rb.metric("Average Growth Rate", f"{pr:.3f} cm/day")
            st.info(f"Confidence range: **{max(0, pl-mae_len):.2f} – {pl+mae_len:.2f} cm**")
            ...
            fc = [{"Day": d, "Phase": get_phase(d),
                   ...
                   "Avg Rate (cm/day)": round(model_rate.predict([[temp,hum,tds,ph,d]])[0], 3)}
...
with tab2:
    ...
    st.caption("Runs the complete lifecycle. At each break point, the AI analyses conditions and applies ALL actionable adjustments (not just one) to continuously optimize growth. We also add tiny daily natural variations to simulate reality.")
    ...
    init_temp = s1.number_input("Temp (°C)",    18.0, 33.5, 28.0, step=0.5)
    init_hum  = s2.number_input("Humidity (%)", 50.0, 80.0, 65.0, step=1.0)
    init_tds  = s3.number_input("TDS (ppm)",    400.0,800.0,580.0,step=10.0)
    init_ph   = s4.number_input("pH",           6.0,  6.8,  6.4,  step=0.1)
    ...
    if run_btn:
        ...
        for day in range(1, MAX_DAYS + 1):
            # Apply tiny daily random variation to simulate real environment
            ...
            if day % break_interval == 0 and day < MAX_DAYS:
                ...
                # FIXED STATIC PROBLEM: Apply ALL actionable suggestions! (Removed the 'break')
                for t in tips:
                    if t["dir"] != "OK":
        ...
        sm2.metric("Peak Rate",         f"{sim_df['rate'].max():.3f} cm/day", f"Day {int(sim_df.loc[sim_df['rate'].idxmax(),'day'])}")
        ...
        for d, l in zip(bdays, blens):
            ax1.annotate(f"d{d}", (d,l), xytext=(2,6), textcoords="offset points", fontsize=7, color="darkorange")
        ...
        ax2.axhline(sim_df["rate"].mean(), color="gray", ls=":", lw=1.2, label=f"Avg {sim_df['rate'].mean():.3f} cm/day")
        ...
        st.markdown("#### 🌡️ Environmental History (Dynamic)")
        st.caption("Watch how the AI auto-corrected multiple sensors at breakpoints, while minor natural fluctuations occurred daily.")
        fig2, ax_env = plt.subplots(figsize=(14, 3))
        ax_env.plot(sim_df["day"], sim_df["temp"], label="Temp (°C)", color="#e63946", lw=1.5)
        ax_env.plot(sim_df["day"], sim_df["hum"]/2, label="Hum (% / 2)", color="#457b9d", lw=1.5)
        ax_env.plot(sim_df["day"], sim_df["tds"]/20, label="TDS (ppm / 20)", color="#2a9d8f", lw=1.5)
        ax_env.set_xlim(1, MAX_DAYS)
        ax_env.set_title("Sensor Adjustments Over Time (Scaled for single view)", fontsize=10)
        ax_env.legend(loc="lower right", fontsize=8, ncol=3)
        ...
        for b in sim_breaks:
            with st.expander(f"{b['phase']}  ·  📍 Day {b['day']}  ·  Length: **{b['pred_len']:.2f} cm**", expanded=False):
                st.markdown("**Sensor readings AT THIS BREAK (before adjustment):**")
                sc = st.columns(4)
                for col, k in zip(sc, KEYS):
                    v = b["sensors"][k]
                    h = health_status(k, v)
                    col.metric(f"{h_icon[h]} {k}", f"{v:.1f}{UNITS[k]}", h_label[h])

                st.markdown("**AI Recommendations APPLIED for next phase:**")
                tc = st.columns(4)
                for col, t in zip(tc, b["tips"]):
                    ...
                    if t["dir"] == "OK":
                        ...
                        col.warning(f"**{k}** ⬆️ Increased\n\nTarget: **{fmt}{UNITS[k]}**")
                    else:
                        col.info(f"**{k}** ⬇️ Decreased\n\nTarget: **{fmt}{UNITS[k]}**")
...
# ╔══════════════════════════════════════════════════════════════════════╗
# ║  MODE 3 — ANALYSIS PLOTS (RESTORED)                                  ║
# ╚══════════════════════════════════════════════════════════════════════╝
with tab3:
    ...
    if plot_choice == "Predicted vs Actual":
        ...
        ax.set_xlabel("Actual Length (cm)")
        ax.set_ylabel("Predicted Length (cm)")
        ax.set_title("Length Model: Predicted vs Actual")
        st.pyplot(fig)
        plt.close(fig)
        ...
        ax.set_title("Feature Importance for Growth Length")
        ax.invert_yaxis()
        ...
        for ax, k in zip(axes, KEYS):
            oLo, oHi = OPTIMA[k]
            ax.axvspan(oLo, oHi, alpha=0.15, color=CLR_PHASE[2], label="Optimal zone")
            ax.scatter(df[k], df["Growth_Length"], alpha=0.3, s=8, color="#1D9E75")
            ax.set_xlabel(f"{k} ({UNITS[k].strip()})", fontsize=9)
            ax.set_ylabel("Length (cm)" if k == "Temp" else "", fontsize=9)
            ax.set_title(k, fontsize=10)
            ax.grid(alpha=0.2)
        fig.suptitle("Sensor Values vs Final Growth Length", fontsize=11, fontweight="bold")
        plt.tight_layout()
        st.pyplot(fig)
        plt.close(fig)
