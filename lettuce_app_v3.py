...
with tab2:
    ...
    if not st.session_state.sim_running and not st.session_state.sim_done:
        ...
        it  = s1.number_input("Temp (C)",    18.0, 33.5, 28.0, step=0.5)
        ih  = s2.number_input("Humidity (%)",50.0, 80.0, 65.0, step=1.0)
        itds= s3.number_input("TDS (ppm)",   400.0,800.0,580.0,step=10.0)
        ip  = s4.number_input("pH",          6.0,  6.8,  6.4,  step=0.1)
    ...
    if plot_section == "Length Model":
        ...
        if plot_choice == "Predicted vs Actual":
            ...
            sns.heatmap(df[cols].corr(), annot=True, cmap="RdYlGn", center=0,
                        fmt=".2f", ax=ax, annot_kws={"size": 9})
            ax.set_title("Correlation Heatmap (All Features)")
            st.pyplot(fig); plt.close(fig)

        elif plot_choice == "Sensor vs Growth Length":
            fig, axes = plt.subplots(1, 4, figsize=(14, 4))
            for ax, k in zip(axes, KEYS):
                oLo, oHi = OPTIMA[k]
                ax.axvspan(oLo, oHi, alpha=0.15, color=CLR_PHASE[2], label="Optimal")
                ax.scatter(df[k], df["Growth_Length"], alpha=0.2, s=6, color="#1D9E75")
        ...
        plot_choice = st.radio("Chart:", [
            ...
            "Sensor vs Growth Rate",
            "Rate Heatmap - Day x Temperature",
        ...
        elif plot_choice == "Sensor vs Growth Rate":
            fig, axes = plt.subplots(1, 4, figsize=(14, 4))
            sc = None
            for ax, k in zip(axes, KEYS):
                oLo, oHi = OPTIMA[k]
                ax.axvspan(oLo, oHi, alpha=0.15, color=CLR_PHASE[2], label="Optimal")
                sc = ax.scatter(df[k], df["Growth_Rate"], c=df["Growth_Days"],
                                cmap="viridis", alpha=0.3, s=8)
                ax.set_xlabel(f"{k} ({UNITS[k].strip()})", fontsize=9)
                ax.set_ylabel("Rate (cm/day)" if k == "Temp" else "", fontsize=9)
                ax.set_title(k, fontsize=10); ax.grid(alpha=0.2)
            if sc:
                plt.colorbar(sc, ax=axes[-1], label="Growth Day")
            fig.suptitle("Sensor Values vs Growth Rate (coloured by Growth Day)",
                         fontsize=11, fontweight="bold")
            plt.tight_layout(); st.pyplot(fig); plt.close(fig)
            st.caption("Darker points = later in lifecycle. Shows how sensor impact on rate changes across growth stages.")

        elif plot_choice == "Rate Heatmap - Day x Temperature":
            st.caption("Average growth rate for each combination of growth day and temperature band. Brighter = faster growth.")
            df_h = df.copy()
            df_h["Temp_bin"] = pd.cut(df_h["Temp"], bins=8, precision=0)
            pivot = df_h.pivot_table(values="Growth_Rate", index="Temp_bin",
                                      columns="Growth_Days", aggfunc="mean")
            fig, ax = plt.subplots(figsize=(16, 5))
            sns.heatmap(pivot, cmap="YlGn", ax=ax, cbar_kws={"label": "Avg Rate (cm/day)"}, linewidths=0.1)
            ax.set_xlabel("Growth Day"); ax.set_ylabel("Temperature Band (C)")
            ax.set_title("Growth Rate Heatmap: Temperature x Growth Day\nEach cell = avg cm/day at that temp on that day")
            plt.tight_layout(); st.pyplot(fig); plt.close(fig)
...
Length model + 5 growth rate charts:
- Rate predicted vs actual
- Feature importance comparison
- Box plot by phase
- Sensor vs rate scatter
- Rate heatmap Day x Temp
